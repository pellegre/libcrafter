...
fn ldpc_corpus() -> Vec<u8> {
    std::fs::read(format!(
        "{}/tests/fixtures/iq/eht-data-ldpc-iq.cs8",
        env!("CARGO_MANIFEST_DIR")
    ))
    .unwrap()
}
...
fn radio_eht_data_ldpc_independent_iq_waveforms() {
    ...
    for row in rows.lines().skip(1) {
        ...
        let input = &samples[acquisition.signal_start as usize..];
        let signal = SignalReceiver::recover(input, &acquisition).expect(columns[0]);
        assert_eq!(signal.legacy_length, columns[12].parse().unwrap());
        let trained = training::Receiver::recover(input, &acquisition, signal).expect(columns[0]);
        assert_eq!(trained.data_start, columns[14].parse().unwrap());
        let admission =
            super::super::Receiver::admit(trained, usize::MAX, usize::MAX).expect(columns[0]);
        assert_eq!(admission.capacity.mcs, columns[3].parse::<u8>().unwrap());
        assert_ne!(admission.capacity.mcs, 13);
        if admission.capacity.bits_per_tone == 12 {
            assert_eq!(columns[16], "0", "{}", columns[0]);
        }
        assert!(admission.capacity.ldpc);
        assert_eq!(admission.timing.data_symbols, columns[8].parse().unwrap());
        assert_eq!(admission.trained.guard, columns[5].parse().unwrap());
        assert_eq!(
            admission.info.end_sample_index,
        ...
        let recovered = super::super::Receiver::recover(admission, input, &acquisition, usize::MAX)
            .expect(columns[0]);
        assert_eq!(recovered.psdu, bytes(columns[11]), "{}", columns[0]);
        assert_eq!(recovered.failed_codewords, 0, "{}", columns[0]);
        assert_eq!(recovered.first_failure, None, "{}", columns[0]);
    }
}
...
fn radio_eht_data_ldpc_streams_raw_mpdu_bytes() {
    ...
    for (index, row) in rows.lines().skip(1).enumerate() {
        ...
        let offset: usize = columns[17].parse().unwrap();
        let length: usize = columns[18].parse().unwrap();
        let expected_psdu = bytes(columns[11]);
        let output = crate::radio::eht::test_support::feed(
            &mut WifiDecoder::new(),
            &corpus[offset..offset + length],
            [1, 37, 128, 997][index % 4],
