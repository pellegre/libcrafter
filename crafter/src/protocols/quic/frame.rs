...
impl QuicMaxDataFrame {
    /// Construct a MAX_DATA frame from a caller-supplied varint.
    pub const fn new(maximum_data: QuicVarInt) -> Self {
        Self { maximum_data }
    }

    /// Construct a MAX_DATA frame from a validated integer value.
    pub fn from_value(maximum_data: u64) -> Result<Self> {
        Ok(Self::new(QuicVarInt::new(maximum_data)?))
    }

    /// Return the Maximum Data field.
    pub const fn maximum_data(self) -> QuicVarInt {
        self.maximum_data
    }

    /// Decode one MAX_DATA frame from bytes that include the Frame Type.
    pub fn decode(bytes: impl AsRef<[u8]>) -> Result<Self> {
        let bytes = bytes.as_ref();
    ...
    /// Append the canonical MAX_DATA frame encoding.
    pub fn encode(self, out: &mut Vec<u8>) -> Result<()> {
    ...
    /// Return the canonical MAX_DATA frame encoding.
    pub fn encode_to_vec(self) -> Result<Vec<u8>> {
        let mut out = Vec::new();
        self.encode(&mut out)?;
        Ok(out)
    ...
    /// Stable MAX_DATA summary for packet inspection.
    pub fn summary(self) -> String {
        format!("kind=MAX_DATA maximum_data={}", self.maximum_data.value())
    }

    /// Stable field/value pairs for MAX_DATA inspection.
    pub fn inspection_fields(self) -> Vec<(&'static str, String)> {
...
/// Parsed DATA_BLOCKED frame fields.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
impl QuicDataBlockedFrame {
    /// Construct a DATA_BLOCKED frame from a caller-supplied varint.
    pub const fn new(data_limit: QuicVarInt) -> Self {
        Self { data_limit }
    }

    /// Construct a DATA_BLOCKED frame from a validated integer value.
    pub fn from_value(data_limit: u64) -> Result<Self> {
        Ok(Self::new(QuicVarInt::new(data_limit)?))
    }

    /// Return the Data Limit field.
    pub const fn data_limit(self) -> QuicVarInt {
        self.data_limit
    }

    /// Decode one DATA_BLOCKED frame from bytes that include the Frame Type.
    pub fn decode(bytes: impl AsRef<[u8]>) -> Result<Self> {
        let bytes = bytes.as_ref();
    ...
    /// Append the canonical DATA_BLOCKED frame encoding.
    pub fn encode(self, out: &mut Vec<u8>) -> Result<()> {
    ...
    /// Return the canonical DATA_BLOCKED frame encoding.
    pub fn encode_to_vec(self) -> Result<Vec<u8>> {
        let mut out = Vec::new();
        self.encode(&mut out)?;
        Ok(out)
    ...
    /// Stable DATA_BLOCKED summary for packet inspection.
    pub fn summary(self) -> String {
        format!("kind=DATA_BLOCKED data_limit={}", self.data_limit.value())
    }

    /// Stable field/value pairs for DATA_BLOCKED inspection.
    pub fn inspection_fields(self) -> Vec<(&'static str, String)> {
...
/// Parsed RETIRE_CONNECTION_ID frame fields.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
impl QuicRetireConnectionIdFrame {
    /// Construct a RETIRE_CONNECTION_ID frame from a caller-supplied varint.
    pub const fn new(sequence_number: QuicVarInt) -> Self {
        Self { sequence_number }
    }

    /// Construct a RETIRE_CONNECTION_ID frame from a validated integer value.
    ...
    /// Decode one RETIRE_CONNECTION_ID frame from bytes that include the Frame Type.
    pub fn decode(bytes: impl AsRef<[u8]>) -> Result<Self> {
        let bytes = bytes.as_ref();
    ...
    /// Append the canonical RETIRE_CONNECTION_ID frame encoding.
    pub fn encode(self, out: &mut Vec<u8>) -> Result<()> {
    ...
    /// Return the canonical RETIRE_CONNECTION_ID frame encoding.
    pub fn encode_to_vec(self) -> Result<Vec<u8>> {
        let mut out = Vec::new();
        self.encode(&mut out)?;
        Ok(out)
    ...
    /// Stable RETIRE_CONNECTION_ID summary for packet inspection.
    pub fn summary(self) -> String {
        format!(
    ...
    /// Stable field/value pairs for RETIRE_CONNECTION_ID inspection.
    pub fn inspection_fields(self) -> Vec<(&'static str, String)> {
        vec![(
...
impl QuicFrame {
    ...
    /// Construct a MAX_DATA frame from typed fields.
    ...
    /// Construct a DATA_BLOCKED frame from typed fields.
    ...
    /// Construct a RETIRE_CONNECTION_ID frame from typed fields.
    ...
    /// Decode this frame as MAX_DATA when applicable.
    ...
    /// Decode this frame as DATA_BLOCKED when applicable.
    ...
    /// Decode this frame as RETIRE_CONNECTION_ID when applicable.
