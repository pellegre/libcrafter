...
#[test]
fn radio_training_window_exact_frame_recovery() {
    use crafter::radio::*;
    let root = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("tests/fixtures/iq");
    let mut matrix = std::collections::BTreeSet::new();
    for line in include_str!("fixtures/iq/ofdm-training-window-index.tsv")
        .lines()
        .skip(1)
    {
        let c: Vec<_> = line.split('\t').collect();
        ...
        assert_eq!(bytes.len(), 2 * c[6].parse::<usize>().unwrap());
        assert_eq!(&bytes[..2 * (37 + 312)], &source[..2 * (37 + 312)]);
