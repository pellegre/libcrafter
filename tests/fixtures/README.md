...
- `scapy/`: reference inputs or outputs generated by Scapy. These fixtures are
  ...
  tool.
- `legacy/`: reference inputs or outputs generated by legacy libcrafter tools or
  examples. These preserve behavior that should carry into the Rust port.
...
## Legacy Libcrafter Fixtures

Legacy fixtures come from the existing C++ library, its examples, or generated
`test.dat` files from the examples checkout. Keep the original behavior visible
without depending on a local legacy build during normal static tests.

Recommended pairs:

```text
bytes/arp--who-has.bin
legacy/arp--who-has.source.txt
legacy/arp--who-has.metadata.json
```

If legacy behavior differs from Scapy or from protocol defaults, document the
reason in the metadata and choose deliberately whether the Rust port preserves
legacy behavior or uses the protocol-correct value.
