...
- `scapy/`: reference inputs or outputs generated by the fixture generator.
  These fixtures are used when comparing Rust encoders/decoders against a known
...
Reference fixtures should record both the generated bytes and enough structured
metadata to explain the intent. When exact byte equality is meaningful, store the
bytes in `bytes/` and keep the recipe or structured output in `scapy/`.
...
generator fills implicitly, such as checksums and lengths, should be called out
in the metadata so Rust tests can assert the same behavior.
