"""SCTP probe protocol plugin: deterministic dry-run peer behavior plans."""
...
SCTP_RUNTIME = "probe-sctp-reference"
...
SCTP_PROBE_CASES: tuple[ProbeCase, ...] = (
    ...
    _behavior_case(
        ...
        description="Plan INIT / INIT ACK association setup with cookie preservation.",
        stimulus="sctp_init",
...
_SCTP_CASE_BY_NAME: dict[str, ProbeCase] = {case.name: case for case in SCTP_PROBE_CASES}
...
def _sctp_probe_plan(
    *,
    ...
    packet_stack = ["ipv4", "udp", "sctp"] if transport == "udp+sctp" else ["ipv4", "sctp"]
    ...
    return {
        ...
        "target_service": {
            "required": True,
            "kind": SCTP_SERVICE_KIND,
            "protocol": transport,
            "port": SCTP_UDP_ENCAP_PORT if transport == "udp+sctp" else destination_port,
            "bind_ipv4": target_ipv4,
            "source_ipv4": source_ipv4,
            "runtime": SCTP_RUNTIME,
            "behavior": message_kind,
            "deterministic": True,
            "planned_only": True,
    ...
    *,
    ...
    if transport == "udp+sctp":
        return (
            ...
            f"and src port {SCTP_UDP_ENCAP_PORT} and dst port {SCTP_UDP_ENCAP_PORT}"
        )
    return (
        f"sctp and src host {target_ipv4} and dst host {source_ipv4} "
...
def sctp_failure_reasons(case_name: str) -> list[str] | None:
    if case_name in _SCTP_CASE_BY_NAME:
        return [
            FAILURE_TIMEOUT,
            FAILURE_WRONG_PEER,
            FAILURE_WRONG_PAYLOAD,
...
def sctp_lab_capabilities(substrate: Mapping[str, JSONValue]) -> Mapping[str, object]:
    ...
    controlled_peer = dry_run or (
        ipv4_unicast
        and controlled_services
        and capability_default_true(substrate, "sctp_controlled_peer", "sctp_peer")
    )
    return {"sctp_controlled_peer": controlled_peer}


_SCTP_PLAN_BUILDERS = {
    case.name: _sctp_probe_plan for case in SCTP_PROBE_CASES
}
...
register(
    ProtocolPlugin(
        ...
        profile_counts={SCTP_SMOKE_PROFILE: {case.name: 1 for case in SCTP_PROBE_CASES}},
        stimulus_endpoint_cases=frozenset(),
        target_service=None,
        setup_script=None,
        rewrite_endpoint_addresses=None,
        failure_reasons=sctp_failure_reasons,
        lab_capabilities=sctp_lab_capabilities,
    )
)
