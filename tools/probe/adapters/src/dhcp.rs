...
/// Build the IPv4/UDP/BOOTP/DHCP Discover stimulus packet with libcrafter.
///
/// The Discover is sent from the DHCP client port (68) to the server port (67).
/// `compile()` fills the BOOTP op/htype/hlen, magic cookie, lengths, and the
/// UDP/IPv4 checksums; the caller-set client MAC, transaction id, and broadcast
/// flag survive untouched. A hostname option (12) is added so the responder can
/// echo the client identity in the Offer.
pub fn dhcp_packet(plan: &ProbePlan) -> ExampleResult<Packet> {
    ...
    // address is configured sets the broadcast flag. The lab transport unicasts
    // the Offer back over the private segment, so the responder uses the
    // recorded source address; the flag stays a faithful Discover shape.
    let dhcp = Dhcp::discover(client_mac).transaction_id(transaction_id);
