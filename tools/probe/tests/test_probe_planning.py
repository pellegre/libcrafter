    ...
    def test_registered_cases_resolve_through_dispatch_not_fallback(self) -> None:
        ...
        for name in planning.PLAN_BUILDERS:
            ...
            if name in planning.PLANNED_ONLY_REGISTERED_CASES:
                ...
                elif case.metadata.get("protocol") == "ssdp":
                    self.assertEqual(plan["protocol"], "ssdp", name)
                    self.assertEqual(plan["destination_port"], 1900, name)
                    self.assertIn("ssdp", plan, name)
                    self.assertIn("expected_ssdp", plan, name)
                    self.assertEqual(
                        plan["stimulus_driver"]["name"],
                        "ssdp_probe",
                        name,
                ...
                elif case.metadata.get("protocol") == "ntp":
                    self.assertEqual(plan["protocol"], "ntp", name)
                    self.assertEqual(plan["destination_port"], 123, name)
                    self.assertIn("ntp", plan, name)
                    self.assertIn("expected_ntp", plan, name)
                    self.assertEqual(
                        plan["stimulus_driver"]["name"],
                        "ntp_probe",
                        name,
                ...
                elif case.metadata.get("protocol") == "coap":
                    self.assertEqual(plan["protocol"], "coap", name)
                    self.assertEqual(plan["stimulus_driver"]["state"], "planned-only", name)
                    self.assertTrue(plan["validation"]["planned_only"], name)
                else:
