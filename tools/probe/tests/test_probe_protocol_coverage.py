...
* Full registry coverage -- every one of the 22 known probe protocols
  (``arp``, ``dns``, ``dhcpv4``, ``dhcpv6``, ``udp``, ``ndp``, ``icmp``,
  ``tcp``, ``bgp``, ``rip``, ``ospf``, ``igmp``, ``ipsec``, ``mqtt``,
  ``quic``, ``snmp``, ``sctp``, ``ssdp``, ``mdns``, ``tls``, ``ntp``, ``coap``) has a registered
...
class ProbeProtocolFullCoverageTest(unittest.TestCase):
    ...
    def test_every_known_protocol_is_registered(self) -> None:
        """Each of the 22 known protocols has a registered plugin."""
    ...
    def test_registry_exactly_matches_known_protocols(self) -> None:
        """The registry is exactly the 22 known protocols -- nothing more, nothing less."""
