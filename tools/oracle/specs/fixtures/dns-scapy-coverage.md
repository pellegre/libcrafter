...
| EDNS option TLV matrix: ordered NSID, COOKIE, Padding, DAU (mnemonic without a constructor), and an unknown option code, raw option data preserved | `dns-edns-options` | both | strict_bytes | partial (`EDNS0TLV(optcode=N, optdata=...)` raw option data) | yes |
...
| Combined NSEC3 (type 50): non-empty salt + non-empty next hashed owner + multiple bitmap entries; empty salt; unknown hash algorithm (0xfe) + unknown high bitmap codepoint (TYPE65280) | `dnssec-nsec3` | both | strict_bytes (salt and hash fields compared as bytes, not text) | yes (`DNSRRNSEC3`, same uncompressed bytes) | yes |
