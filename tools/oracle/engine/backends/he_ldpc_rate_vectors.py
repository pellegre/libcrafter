                ...
                for group in ([1,2] if nss==1 and not dcm else [1]):
                    for initial in sorted(set(range(group,65,group))|{n for n in (127,128,129,199,200,399,400) if n%group==0}):
                        for padding in range(1,5):
                            result=layout(mcs,nss,dcm,group,initial,padding,er_tones)
...
def er_codewords(tones):
    rows=['mcs\tdcm\tgroup\tsymbols\tpadding\textra\tpayload_bits\ttransmitted_bits']
    for mcs in range(1 if tones==106 else 3):
        for dcm in ([0,1] if mcs in (0,1) else [0]):
            for group in ([1,2] if not dcm else [1]):
                for initial in (group*k for k in (1,4,13,26,64)):
                    for padding in range(1,5):
                        sizing=layout(mcs,1,dcm,group,initial,padding,tones)
                        symbols,pad,extra,*_=sizing
                        payload=sizing[8]
                        if payload<16 or symbols>400:continue
                        raw=hashlib.shake_256(f'he-er{tones}-ldpc-{mcs}-{dcm}-{group}-{initial}-{padding}'.encode()).digest((payload+7)//8)
                        bits=''.join(str(b>>i&1) for b in raw for i in range(8))[:payload]
                        coded=''.join(map(str,encode_information(bits,sizing,mcs)))
                        rows.append('\t'.join(map(str,[mcs,dcm,group,symbols,pad,extra,bits,coded])))
    return '\n'.join(rows)+'\n'
...
if __name__=='__main__':
    ...
    parser.add_argument('--er-tones',type=int,choices=(106,242));args=parser.parse_args()
    prefix=f'he-er{args.er_tones}' if args.er_tones else 'he'
    for name,content in [(f'{prefix}-ldpc-rate-index.tsv',generate(args.er_tones)),
                         (f'{prefix}-ldpc-rate-codewords.tsv',er_codewords(args.er_tones) if args.er_tones else codewords())]:
