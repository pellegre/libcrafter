...
def generate(out):
                ...
                for branch in (0,1,2):
                    ...
                    for j in range(count):
                        ...
                        for k in data+pilots:
                            h0 = 0 if branch==1 else (1+.25j)*cmath.exp(-2j*math.pi*k*3/256)
                            h1 = 0 if branch==2 else (.6-.3j+.2j*cmath.exp(-2j*math.pi*k*5/256))*cmath.exp(2j*math.pi*k*8/256)
                    ...
                    name = f'he-mu-stbc-training-ru{ru}-index{index}-ltf{size}-gi{guard*50}-n{count}-branch{branch}'
    ...
    (out/'he-mu-stbc-training-index.tsv').write_text('\n'.join(rows)+'\n')
