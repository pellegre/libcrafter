...
OUT=Path(__file__).resolve().parents[4]/'crafter/tests/fixtures/iq/he-dcm-metrics.tsv'
...
def pair(bits,k):
    ...
    lower=point(bits)
    if len(bits)==1:upper=(lower[0]*(-1 if (k+117)%2 else 1),0)
...
def generate():
    ...
    for bps in (1,2,4):
        ...
        for k in (0,1,116):
            for label in labels:
                for noise in (0,1):
                    observed=[F(v)+noise*d for v,d in zip(pair(label,k),[F(1,8),F(1,4),F(-3,8),F(-1,8)])]
                    for weights in ((F(1),F(1)),(F(0),F(1)),(F(1),F(0)),(F(1,8),F(4)),(F(0),F(0))):
                        costs={b:sum(w*(o-v)**2 for o,v,w in zip(observed,pair(b,k),[weights[0]]*2+[weights[1]]*2))/energy for b in labels}
