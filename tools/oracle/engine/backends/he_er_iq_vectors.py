...
def waveform(mode,mcs,ldpc,size,guard,padding,invalid=None):
    period=10 if padding==3 else 20 if padding==4 else None
    case='changing' if period else 'selective'
    if mode=='stbc':
        return stbc_wave(mcs,size,guard,ldpc,padding,case,period,invalid,er=True)
    ...
    iq,f=single(mcs,size,guard,case,invalid=invalid,payload=wire,ldpc=ldpc,
                ...
                initial_symbols=2*period+2 if period else 5,dcm=mode=='dcm',er=True)
...
def generate(out):
    ...
    for mode in ('plain','dcm','stbc'):
        ...
        for ldpc in (False,True):
            for mcs in range(2 if mode=='dcm' else 3):
                for size,guard in guards:
                    for padding in (1,2,3,4):
                        name=f'he-er-iq-{mode}-mcs{mcs}-{"ldpc" if ldpc else "bcc"}-ltf{size}-gi{guard*50}-pad{padding}'
                        iq,psdu,frames,start,end=waveform(mode,mcs,ldpc,size,guard,padding)
            ...
            for error in errors:
                name=f'he-er-iq-invalid-{mode}-{"ldpc" if ldpc else "bcc"}-{error}'
                iq,*_=waveform(mode,0,ldpc,4,64,3,invalid=error)
                (out/f'{name}.cs8').write_bytes(iq)
                invalid.append(f'{name}\t{hashlib.sha256(iq).hexdigest()}')
    (out/'he-er-iq-index.tsv').write_text('\n'.join(rows)+'\n')
    (out/'he-er-iq-invalid-index.tsv').write_text('\n'.join(invalid)+'\n')
    print(f'{len(rows)-1} complete ER242 waveforms; {len(invalid)-1} invalid cases')
