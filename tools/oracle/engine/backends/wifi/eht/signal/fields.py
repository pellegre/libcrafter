"""Independent EHT-SIG encoding blocks.
...
if __name__ == "__main__":
    ...
    inventories = (
        (OUT, generate_single(), "EHT-SIG SU inventory differs"),
        (MU_OUT, generate_multi(), "EHT-SIG MU inventory differs"),
        (OFDMA_OUT, generate_ofdma(), "EHT-SIG OFDMA inventory differs"),
    )
    for path, result, failure in inventories:
        if args.write:
            path.write_text(result)
        else:
            assert path.read_text() == result, failure
    print(
        f"2048 single-user, 1792 MU-MIMO and {len(OFDMA_USERS) * 16} "
        "OFDMA EHT-SIG block chains verified"
    )
