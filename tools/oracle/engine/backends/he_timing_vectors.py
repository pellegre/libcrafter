"""Independent forward HE SU timeline, IEEE802.11ax-2021 27.3.12.16/13.
...
OUT = Path(__file__).resolve().parents[4]/'crafter/tests/fixtures/iq/he-timing-index.tsv'
...
if __name__ == '__main__':
    ...
    content = generate()
    if args.check:
        assert OUT.read_text()==content,'HE forward timeline differs'
    else:
        OUT.write_text(content)
    print(f'{len(content.splitlines())-1} independent HE SU timing cases')
