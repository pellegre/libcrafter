...
DHCPv4 carries a feature behavior like IPv4/UDP/DNS: ``apply_behavior`` owns
the ``dhcpv4_behavior`` feature branch and
``handles_feature`` claims ownership of the ``"dhcpv4_behavior"`` feature name, so
the generator's registry-first feature loop runs it exactly once.

``DHCPV4_OPTION_MATRIX_TOKENS`` stays generator-visible because
``tools/oracle/tests/test_dhcpv4_oracle.py`` imports it from ``generator``. It is
...
# DHCPv4 fields the generator samples.
...
# Backend-neutral DHCPv4 option kinds that materialize byte-for-byte through both
# the Scapy reference backend and the libcrafter adapter, in fixed option order.
# Kinds the dhcpv4_behavior option_matrix lists that Scapy cannot encode
...
# tools/oracle/specs/layers/dhcpv4.yaml and features/dhcpv4-behavior.yaml.
...
def _dhcpv4_option_matrix() -> list[object]:
    ...
    for both directions and both DHCPv4 roots.
    """
...
def _apply_behavior(
    ...
    *,
    ...
    """Apply the ``dhcpv4_behavior`` feature behavior to sampled DHCPv4 fields.

    ``grammar`` is part of the uniform ``apply_behavior`` call path (the TCP
    ``tcp_header`` behavior reads it); DHCPv4 does not consult it.
    """
...
def _handles_feature(feature: str) -> bool:
    return feature == "dhcpv4_behavior"
