...
class NtpOracleSpecTest(unittest.TestCase):
    def test_specs_declare_ntp_layer_features_profiles_and_stacks(self) -> None:
        ...
        for profile_name in ("ntp-smoke", "ntp-ci", "ntp-live-dry-run"):
    ...
    def test_sampler_and_backend_plugins_match_ntp_field_surface(self) -> None:
        layer_fields = {field.name for field in load_oracle_specs().layers["ntp"].fields}
...
class NtpGeneratorTest(unittest.TestCase):
    def test_seeded_ntp_generation_is_deterministic_and_uses_ntp_stacks(self) -> None:
        first = generate_plans(seed=_SEED, profile="ntp-smoke", count=8, backend="scapy")
        second = generate_plans(seed=_SEED, profile="ntp-smoke", count=8, backend="scapy")
        ...
        for plan in first:
            with self.subTest(case=plan.case, stack=plan.stack):
                ...
                self.assertIn(123, {plan.fields["udp"]["src_port"], plan.fields["udp"]["dst_port"]})
                self.assertIn("ntp", plan.fields)

    def test_explicit_header_plan_matches_documentation_address_udp_context(self) -> None:
...
class NtpScapyBackendTest(unittest.TestCase):
    ...
    def test_materializer_preserves_extension_and_legacy_mac_tail(self) -> None:
        ...
        self.assertEqual(normalized["extension_fields"][0]["extension_field_type"], 0x0204)
    ...
    def test_raw_fallback_payload_is_materialized_but_not_promoted_to_ntp(self) -> None:
        ...
        self.assertEqual(fields["payload"], {"hex": payload.hex(), "length": len(payload)})
...
class NtpWiresharkBackendTest(unittest.TestCase):
    def test_source_byte_normalization_parses_valid_ntp_payload(self) -> None:
        ...
        self.assertEqual(normalized["first_octet"], 0x23)
        self.assertEqual(normalized["extension_count"], 1)
        self.assertEqual(normalized["extension_fields"][0]["extension_field_type"], 0x0404)
        self.assertEqual(normalized["extension_fields"][0]["nts_extension"], "authenticator")
        self.assertTrue(normalized["extension_fields"][0]["authenticator"]["parts_present"])

    def test_source_byte_normalization_rejects_wrong_udp_port_and_malformed_payload(self) -> None:
        valid = ntp_scapy._ntp_message_bytes(
            _ntp_plan("ntp-header-client-request", "ntp_header").fields
        )
        wrong_port_udp_datagram = (
            b"\xc0\x00\xc0\x01" + (len(valid) + 8).to_bytes(2, "big") + b"\x00\x00" + valid
        )
        ...
        self.assertEqual(ntp_wireshark._normalize_ntp({}, source_hex=raw_fallback.hex()), {})
