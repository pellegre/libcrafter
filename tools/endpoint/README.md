...
`tools/endpoint` is the shared endpoint layer for live provider work. It creates,
tracks, uses, and destroys one runnable endpoint at a time for callers such as
`tools/oracle`, `tools/probe`, and agents.

`tools/endpoint` owns only mechanical endpoint lifecycle:
...
explicitly. `tools/endpoint` does not select providers automatically and does not
...
the user running `tools/endpoint` to use that daemon. Treat Docker daemon or socket
...
tools/endpoint/run doctor --provider qemu --exposure wan --json
...
```sh
python3 tools/endpoint/smoke/live_virtualbox_network_ping.py --plan-only
```
...
python3 tools/endpoint/smoke/live_virtualbox_network_ping.py \
  --live \
...
`tools/endpoint/artifacts/live-virtualbox-network-ping/`.
