...
- **Ambiguous** - current behavior needs source reconciliation before it should
  guide code changes.
- **Out of scope** - explicitly excluded by the manifest and internal workflow scope.
...
| Extension-header chain traversal | RFC 8200; RFC 7045 | `append_ipv6_next_with_registry` | Partial | Routing(43) and Fragment(44) are traversed. Hop-by-Hop(0), Destination Options(60), AH(51), ESP(50), Mobility Header(135), HIP(139), Shim6(140), and experimental extension headers are not typed. No extension-order validation is performed. |
...
| Home Address Destination Option | RFC 6275 | none | Missing | Mobile IPv6 Type 2 routing exists, but Home Address option 0xC9 does not. |
...
| AH and ESP cryptography | IANA extension-header types; manifest exclusion | raw fallback only | Unsupported | Next Header 50/51 payloads are not typed and no crypto, authentication, key management, or verification is implemented. Packet-layer raw preservation does not imply AH/ESP support. |
