...
| Extension-header chain traversal and extension order preservation | RFC 8200; RFC 7045 | `append_ipv6_next_with_registry`; `Layer` composition | Partial | Hop-by-Hop(0), Destination Options(60), Routing(43), and Fragment(44) are traversed when typed. RFC 8200's recommended extension order is guidance for callers; libcrafter does not silently reorder user-composed layers, and public tests cover recommended and unusual order preservation. AH(51), ESP(50), Mobility Header(135), HIP(139), Shim6(140), and experimental extension headers remain untyped/raw. |
